...
def make_anim(name, ticks, colors, do_palette=False):
    anims = {
    ...
    if do_palette:
        ...
        f = Image.new("RGB", (ticks + 10, 16), (0, 0, 0))
        f.paste(strip, (5, 0))
...
def make_modes(ticks):
    ...
    for i, [mode, anim1, anim2] in enumerate(anims):
        ...
        p1 = make_anim(anim1[0], ticks, anim1[1], True)
        p2 = make_anim(anim2[0], ticks, anim2[1], True)
        f.paste(p1, (0, 0 + 16))
        f.paste(p2, (0, 96 + 16))
        d = ImageDraw.Draw(f)
        d.text((5, 4), "%s: %s -> %s" % (mode, anim1[0], anim2[0]), font=font)
